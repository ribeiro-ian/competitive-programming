...
int dfs(int x) {
  ...
  for (int i = 0; i < ceil(adj[x].size() * t / 100.0); i++) total += dfs(adj[x][i]);
...
int main() {
  ...
  while (1) {
    ...
    f(0);
    for (auto &v : adj) sort(v.begin(), v.end(), cmp);
    ...
    for (auto i : adj[0]) {
      fprintf(stderr, "%i: ", i);
      for (auto v : adj[i]) { fprintf(stderr, "%i ", v); }
      fprintf(stderr, " | tam: %i\n", empregados[i]);
    }
