/*
  Maratona Mineira 2025 - C - Cartas
...
void definirPontos() {
  ...
  for (int i = 0; i < v.size(); ++i) { pontos[v[i]] = i + 1; }
}

void printCartas() {
  for (int i = 0; i < J; ++i) {
    fprintf(stderr, "%c: ", 'A' + i);
    for (int j = 0; j < C; ++j) cerr << jog[i][j] << ' ';
    cerr << '\n';
  }
  cerr << '\n';
}
