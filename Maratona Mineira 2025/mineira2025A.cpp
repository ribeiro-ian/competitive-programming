/*
  Maratona Mineira 2025 - A - Antissocial
...
int main() {
  ...
  for (int num = 3; num <= n; ++num, ++j) {
    ...
    if (!ans[meio(l, m)]) q.push(make_pair(tamanho(l, m), make_pair(-l, -m)));
    if (!ans[meio(m, r)]) q.push(make_pair(tamanho(m, r), make_pair(-m, -r)));
    q.pop();
  }
