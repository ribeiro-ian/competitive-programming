...
struct Line {
  ...
  double getSlope() {
    if (a.x == b.x) // Vertical line return INF;
...
bool hasIntersection() {
  ...
  if (areCollinear(l1, l2)) return true;

  // one point intersect
  if (l1.getSide(c) == -l1.getSide(d) && l2.getSide(a) == -l2.getSide(b)) return true;
