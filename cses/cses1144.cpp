...
void build() {
  ...
  for (int i = 0; i < MAX; ++i) tree[i + ST] = range[i];

  for (int i = ST - 1; i >= 1; --i) tree[i] = tree[2 * i] + tree[2 * i + 1];
}
...
void update(ll idx, ll x) {
  ...
  for (int i = idx / 2; i >= 1; i /= 2) tree[i] = tree[2 * i] + tree[2 * i + 1];
}
...
int main() {
  ...
  while (q--) {
    ...
    if (op == '!') {
      ...
      if (c(b) != cA) ans += brute(max(a, SZ * cB + 1), b);
